...
* CMake >= 3.7
* At least one of [MKL](https://www.intel.com/content/www/us/en/developer/tools/oneapi/onemkl.html), [Accelerate](https://developer.apple.com/documentation/accelerate), [DNNL](https://github.com/oneapi-src/oneDNN), and [OpenBLAS](https://www.openblas.net/). Most systems have them pre-installed, so you may need to do anything. However, if you see the error `No BLAS library found` when using LeanInfer, it means you don't have any of these libraries. In that case, we recommend MKL for Intel CPUs, Accelerate for Apple, and DNNL for AMD CPUs. OpenBLAS is cross-platform and can be used as a last resort if you cannot install the others.
